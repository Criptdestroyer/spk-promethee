...
class MY_Controller extends CI_Controller
{
	...
	protected function upload($directory, $tag_name = 'userfile', $max_size = 0)
	{
		if ( isset( $_FILES[$tag_name] ) && !empty( $_FILES[$tag_name]['name'] ) )
		{
			$upload_path = realpath(FCPATH . $directory . '/');
			$config = [
				'allowed_types'		=> 'jpg|png|bmp|jpeg|doc|docx|xls|xlsx|ppt|pptx|pdf',
				'upload_path'		=> $upload_path,
				'max_size'			=> $max_size
			];
			$this->load->library('upload');
			$this->upload->initialize($config);
			return $this->upload->do_upload($tag_name) ? $this->upload->data() : FALSE;
		}
		return FALSE;
	}
